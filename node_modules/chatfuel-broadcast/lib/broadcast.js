'use strict';

var _isHex = require('is-hex');

var _isHex2 = _interopRequireDefault(_isHex);

var _requestPromise = require('request-promise');

var _requestPromise2 = _interopRequireDefault(_requestPromise);

var _url = require('url');

var _url2 = _interopRequireDefault(_url);

var _validateFacebookTags = require('./validateFacebookTags');

var _validateFacebookTags2 = _interopRequireDefault(_validateFacebookTags);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

// http://docs.chatfuel.com/broadcasting/broadcasting-documentation/broadcasting-api

var CHATFUEL_BASE_URL = 'https://api.chatfuel.com';

var validateExpectedParameters = function validateExpectedParameters(options) {
    var expectedParameters = ['botId', 'token', 'userId', 'messageTag'];

    if (!options) {
        throw new Error('Expected options to be passed');
    }

    expectedParameters.forEach(function (parameter) {
        if (!options[parameter]) {
            throw new Error('Expected ' + parameter + ' to be passed');
        }
    });
};

var getBlockIdOrNameFromOptions = function getBlockIdOrNameFromOptions(options) {
    var blockId = options.blockId,
        blockName = options.blockName;


    if (!blockName && !blockId) {
        throw new Error('Expected either blockId or blockName to be passed');
    }

    if (blockName && blockId) {
        throw new Error('Expected blockId or blockName to be passed but not both');
    }

    if (blockId) {
        if (!(0, _isHex2.default)(blockId)) {
            throw new Error('Expected blockId to contain a hexadecimal value');
        }

        return { chatfuel_block_id: blockId };
    }

    return { chatfuel_block_name: blockName };
};

var createChatfuelBroadcastUrl = function createChatfuelBroadcastUrl(botId, userId) {
    return CHATFUEL_BASE_URL + '/bots/' + botId + '/users/' + userId + '/send';
};

var broadcast = function broadcast(options) {
    validateExpectedParameters(options);

    var botId = options.botId,
        token = options.token,
        userId = options.userId,
        attributes = options.attributes,
        messageTag = options.messageTag;


    var chatfuelRedirectBlock = getBlockIdOrNameFromOptions(options);

    if (!(0, _validateFacebookTags2.default)(messageTag)) {
        throw new Error('Invalid Facebook message tag \'' + messageTag + '\'');
    }

    var chatfuelBroadcastUrl = createChatfuelBroadcastUrl(botId, userId);

    var query = Object.assign({}, {
        chatfuel_token: token,
        chatfuel_message_tag: messageTag
    }, chatfuelRedirectBlock, attributes);

    var chatfuelApiUrl = _url2.default.format({
        pathname: chatfuelBroadcastUrl,
        query: query
    });

    var requestOptions = {
        uri: chatfuelApiUrl,
        headers: {
            'Content-Type': 'application/json'
        },
        json: true
    };

    return _requestPromise2.default.post(requestOptions);
};

module.exports = broadcast;